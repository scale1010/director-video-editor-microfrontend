...
class Text extends Resizable {
	...
	public drawTextIdentity(ctx: CanvasRenderingContext2D) {
		// Convert any HTML in the text to a plain-text label for the timeline
		// so that tags like <div> or entities like &nbsp; are not shown.
		const getPlainLabel = (html: string): string => {
			const tmp = document.createElement("div");
			tmp.innerHTML = html ?? "";
			const textContent = tmp.textContent || tmp.innerText || "";
			// Replace multiple whitespace/newlines with single space for compact label
			return textContent.replace(/\s+/g, " ").trim();
		};
		const label = getPlainLabel(this.text);
